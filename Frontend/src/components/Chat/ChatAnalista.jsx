...
export default function ChatAnalista() {
  ...
  const consultar = async () => {
    ...
    setLoading(true);
    try {
      const r = await agenteAnalista(preguntaActual);
      setRespuesta(r);
      ...
      const errorMsg = {
        tipo: "asistente",
        contenido:
          "Lo siento, ocurrió un error al analizar. Por favor intenta de nuevo.",
        timestamp: new Date(),
      };
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header mejorado */}
      <div className="relative mb-8 overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 p-6 shadow-xl">
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-indigo-100 text-sm font-medium">
              Análisis inteligente de datos y tendencias
            </p>
          </div>
        </div>
      </div>
                  ...
                  <div
                    className={`relative group rounded-2xl px-5 py-4 shadow-lg transition-all duration-300 ${
                      mensaje.tipo === "usuario"
                        ...
                        : "bg-white text-gray-800 border-2 border-indigo-100 rounded-tl-sm hover:border-indigo-200 hover:shadow-xl"
                    }`}
                  >
                    {mensaje.tipo === "asistente" && (
                      <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/0 via-indigo-500/10 to-indigo-500/0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                    )}
            ...
            {loading && (
              <div className="flex items-start gap-4 animate-fadeIn">
                ...
                <div className="flex flex-col items-start max-w-[75%]">
                  <div className="relative bg-white border-2 border-indigo-100 rounded-2xl rounded-tl-sm px-5 py-4 shadow-lg">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <FaSpinner className="text-indigo-600 animate-spin text-lg" />
      ...
      {/* Input mejorado */}
      <div className="relative">
        <div className="flex gap-3 items-end bg-white rounded-2xl border-2 border-gray-200 shadow-lg p-4 hover:border-indigo-300 transition-colors">
          <div className="flex-1 relative">
            <input
              ...
              className="w-full px-4 py-3 pr-12 bg-gray-50 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-gray-800 placeholder-gray-400"
              disabled={loading}
            />
