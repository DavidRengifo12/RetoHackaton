...
export default function ChatInventario() {
  ...
  const consultar = async () => {
    ...
    setLoading(true);
    try {
      const r = await agenteInventario(preguntaActual);
      setRespuesta(r);
      ...
      const errorMsg = {
        tipo: "asistente",
        contenido:
          "Lo siento, ocurrió un error al consultar. Por favor intenta de nuevo.",
        timestamp: new Date(),
      };
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          <div className="relative h-full overflow-y-auto p-6 space-y-6 chat-scrollbar">
                ...
                <div
                  className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center shadow-lg ${
                    mensaje.tipo === "usuario"
                      ...
                      : "bg-gradient-to-br from-[#002f19] to-[#001a0e] text-white"
                  }`}
                >
                  {mensaje.tipo === "usuario" ? (
                ...
                >
                  <div
                    className={`relative group rounded-2xl px-5 py-4 shadow-lg transition-all duration-300 ${
                      mensaje.tipo === "usuario"
                        ? "bg-gradient-to-br from-[#002f19] to-[#004529] text-white rounded-tr-sm"
                        : "bg-white text-gray-800 border-2 border-[#E8F5E8] rounded-tl-sm hover:border-[#002f19] hover:shadow-xl"
                    }`}
                  >
                    {mensaje.tipo === "asistente" && (
                      <div className="absolute inset-0 bg-gradient-to-r from-[#002f19]/0 via-[#002f19]/10 to-[#002f19]/0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                    )}
            ...
            {loading && (
              <div className="flex items-start gap-4 animate-fadeIn">
                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-gradient-to-br from-[#002f19] to-[#004529] text-white flex items-center justify-center shadow-lg">
                  <FaBox className="text-sm" />
                </div>
                <div className="flex flex-col items-start max-w-[75%]">
                  <div className="relative bg-white border-2 border-[#E8F5E8] rounded-2xl rounded-tl-sm px-5 py-4 shadow-lg">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <FaSpinner className="text-[#002f19] animate-spin text-lg" />
                        <div className="absolute inset-0 bg-[#E8F5E8] rounded-full animate-ping opacity-20"></div>
                      </div>
      ...
      {/* Input mejorado */}
      <div className="relative">
        <div className="flex gap-3 items-end bg-white rounded-2xl border-2 border-gray-200 shadow-lg p-4 hover:border-[#002f19] transition-colors">
          <div className="flex-1 relative">
            <input
              ...
              className="w-full px-4 py-3 pr-12 bg-gray-50 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-[#002f19] focus:border-[#002f19] transition-all disabled:opacity-50 disabled:cursor-not-allowed text-gray-800 placeholder-gray-400"
              disabled={loading}
            />
