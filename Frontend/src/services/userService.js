...
export const userService = {
  ...
  getRoles() {
    return {
      data: [
        ...
        { nombre: 'administrador', descripcion: 'Usuario con permisos completos' }
      ],
      error: null
    };
  },
  ...
  async createUser(userData) {
    try {
      ...
      if (authError || !supabase.auth.admin) {
        ...
        const { error: updateError } = await this.updateUserRole(signUpData.user.id, userData.rol);
        
        if (updateError) {
          toastService.warning('Usuario creado pero error al asignar rol. Se asignó rol "usuario" por defecto.');
        } else {
      ...
      if (authData.user) {
        ...
        const { error: updateError } = await this.updateUserRole(authData.user.id, userData.rol);
  ...
  async toggleUserStatus(userId, activo) {
    try {
      ...
      if (error) throw error;
      toastService.success(`Usuario ${activo ? 'activado' : 'desactivado'} exitosamente`);
      return { data, error: null };
    } catch (error) {
      toastService.error(`Error al cambiar estado del usuario: ${error.message}`);
      return { data: null, error };
    }
  },
};
